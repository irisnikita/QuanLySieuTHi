...
const listnccReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case types.GET_ONE_NCC:
            ...
            if (targetIndex2 >= 0) {
                return produce(state, draftState => {
                    draftState.editStaff = state.listncc[targetIndex2];
                });
            }
            return state;
        case types.ADD_NCC:
            return produce(state, draftState => {
                draftState.listncc.push(action.payload.staff);
            });
        case types.UPDATE_NCC:
            const targetIndex3 = state.listncc.findIndex(item => item.id === action.payload.staff.id);
            if (targetIndex3 >= 0) {
                console.log(action.payload.staff);
                return produce(state, draftState => {
                    draftState.listncc[targetIndex3] = action.payload.staff;
                });
            }
            return state;
